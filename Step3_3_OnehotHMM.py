...
LMlen=1024
INPUTS_SIZE = len(featuresetN)-11
LABELS_NUM = 8
...
# 72.87%, 84.59, 76.60, 64.89, 19.48, 32.08 # one-hot + HMM  modelpara-HMM-1.pth
# 69.92%, 83.67, 76.01, 43.64, 22.11, 35.75
# 72.61%, 84.07, 71.51, 62.01, 19.15, 32.43
# 69.46%, 82.00, 60.95, 66.87, 22.35, 37.73
...
def make_tensors(data):
    inputs = data[:, :, LMlen:(len(featuresetN)+LMlen-11)]
    UnknownSeq = data[:, :, -1]
...
# dir=r'.\parameter\modelpara-HMM-1.pth'
checkpoint = torch.load(dir)
CRNNmodel.load_state_dict(checkpoint['net'])
acc4 = testModel513N()
acc5 = testModelTS115()
