...
LMlen=1024
INPUTS_SIZE = LMlen+len(featuresetN)-31
LABELS_NUM = 8
...
# 77.69%, 87.67, 81.59, 68.05, 17.98, 27.71 # one-hot + LM  modelpara-LM-1.pth
# 74.86%, 86.82, 81.59, 49.05, 20.72, 31.16
# 77.54%, 87.23, 75.63, 67.94, 17.62, 28.09
# 71.04%, 82.73, 65.22, 67.27, 21.52, 36.53
...
def make_tensors(data):
    inputs = data[:, :, :(len(featuresetN)+LMlen-31)]
    UnknownSeq = data[:, :, -1]
...
# dir=r'.\parameter\modelpara-LM-1.pth'
checkpoint = torch.load(dir)
CRNNmodel.load_state_dict(checkpoint['net'])
acc4 = testModel513N()
acc5 = testModelTS115()
